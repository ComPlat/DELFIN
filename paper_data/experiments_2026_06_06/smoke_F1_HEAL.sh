#!/bin/bash
# Smoke 500 — ULTIMATE config + F1-HEAL (structqual gate + no last-resort)
# Test hypothesis: structqual gate kills heavy_collapse/extra_fragments
# while keeping F1's coverage win (88% native rate).
set -e
cd /home/qmchem_max/ComPlat/DELFIN
LABEL="SMOKE-F1-HEAL-2026-06-06"
...
# D1.v2 + full ULTIMATE morgen-flags + F1 (KEEP COVERAGE)
...
# *** F1-HEAL (2026-06-06): orthogonal structqual hard-gate + no last-resort ***
...
echo "=== SMOKE F1-HEAL $LABEL ==="
date
echo "GUPPY HEAD: $(git log -1 --format=%h)"
echo "F1_COVERAGE: ${DELFIN_FFFREE_F1_COVERAGE} | STRUCTQUAL_GATE: ${DELFIN_FFFREE_F1_STRUCTQUAL_GATE} | NO_LAST_RESORT: ${DELFIN_FFFREE_F1_NO_LAST_RESORT}"
...
echo ""
echo "=== Structqual rejection breakdown ==="
grep -c "f1_structqual_rejected" /tmp/${LABEL}_reason.tsv 2>/dev/null || echo "0 rejected"
echo ""
...
echo ""
echo "=== iter_gate vs ULTIMATE intersect (which we beat at +46 without heal) ==="
...
echo ""
echo "=== iter_gate vs F1-OFF smoke (control: did we gain coverage?) ==="
PYTHONHASHSEED=0 /home/qmchem_max/micromamba/envs/delfin/bin/python scripts/iter_gate.py \
  $LABEL \
  --prev SMOKE-F1-OFF-2026-06-06 2>&1 | head -60
echo "=== F1-HEAL Bisect DONE $(date) ==="
