...
- **Auto OCCUPIER own tree mode**: Build a custom adaptive tree per CONTROL via `OCCUPIER_tree=own`; the runtime generator reads `OCCUPIER_sequence_profiles` and stores the resulting tree as `own_auto_tree.json` in the workspace
