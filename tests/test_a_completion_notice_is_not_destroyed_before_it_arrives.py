...
def test_the_registry_read_modify_write_is_locked_across_processes(tmp_path):
    """The lock must be an OS-level one; a threading lock orders one process.

    This used to read the module's source and look for the word ``flock``
    inside a function found by name. It broke the moment the function was
    renamed -- and it would have passed just as happily on a ``flock`` in a
    comment. What the property actually says is that while a cycle holds
    the lock, nobody else can take it, so that is what is asked here: a
    second, independent open file description, which is what another
    process has.
    """
    import fcntl

    state = tmp_path / "bash_jobs.json"
    sidecar = Path(str(state) + BJ._LOCK_SUFFIX)
    with BJ.cross_process_lock(state):
        assert sidecar.exists(), "the lock must live in a sidecar, not the data"
        other = open(sidecar, "a+")
        try:
    ...
    # And it is released again, or the first cycle would wedge every later one.
    after = open(sidecar, "a+")
    try:
