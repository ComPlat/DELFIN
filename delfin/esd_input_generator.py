...
def calculate_dele_cm1(state1_file: str, state2_file: str) -> Optional[float]:
    """Calculate energy difference DELE in cm^-1 from two ORCA output files.

    DELE = E(initial) - E(final) with sign:
    - Positive: downward transition (initial state higher in energy)
    - Negative: upward transition (reverse ISC/IC, initial state lower in energy)

    Args:
        state1_file: Path to initial state output file (e.g., "S1.out")
        state2_file: Path to final state output file (e.g., "T1.out")

    Returns:
        Energy difference in cm^-1 with sign, or None if energies cannot be extracted
    """
    from delfin.energies import find_electronic_energy

    e_initial = find_electronic_energy(state1_file)
    e_final = find_electronic_energy(state2_file)

    if e_initial is None or e_final is None:
        logger.warning(
            f"Cannot calculate DELE: missing energies from {state1_file} or {state2_file}"
        )
        return None

    # Energy difference in Hartree: E(initial) - E(final)
    # Positive if initial state is higher, negative if lower (reverse ISC/IC)
    delta_e_hartree = e_initial - e_final

    # Convert to cm^-1 (preserve sign)
    delta_e_cm1 = delta_e_hartree * HARTREE_TO_CM1

    logger.info(
        f"DELE calculated: {delta_e_cm1:.2f} cm⁻¹ (E_initial - E_final) from {state1_file} and {state2_file}"
    )
...
def create_isc_input(
    ...
    if dele is None:
        logger.warning(f"DELE could not be calculated for {isc_pair}, using default 1692 cm⁻¹")
        dele = 1692.0
...
def create_ic_input(
    ...
    if dele is None:
        logger.warning(f"DELE could not be calculated for {ic_pair}, using default 1692 cm⁻¹")
        dele = 1692.0
