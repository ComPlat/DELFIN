...
# Thread-local context to expose current job ID to callers (used by OCCUPIER)
_job_context = threading.local()


def get_current_job_id() -> Optional[str]:
    """Return the ID of the currently executing pool job (if any)."""
    return getattr(_job_context, "current_job_id", None)


def _set_current_job_id(job_id: Optional[str]) -> None:
    """Internal helper to set the current job ID in thread-local context."""
    ...
    def _start_job(self, job: PoolJob, allocated_cores: int):
        ...
        with self._lock:
            ...
            def execute_with_cores():
                # Expose current job ID for child detection
