...
def create_tab(ctx, calc_refs=None, archive_refs=None):
    ...
    scheduling_section = widgets.VBox(
        [
            widgets.HTML(
                '<div style="color:#455a64; line-height:1.55;">'
                '<b>SLURM memory headroom</b><br>'
                'Upper bound on the pool memory budget, expressed as a fraction '
                'of <code>SLURM_MEM_PER_NODE</code>. The pool admits ORCA '
                'jobs only while their combined <code>%maxcore × cores</code> '
                'request stays below this cap. The remainder is reserved for '
                'process overhead beyond ORCA\'s integral buffer (basis-set '
                'cache, density storage, MPI inter-rank buffers, glibc heap '
                'fragmentation, periodic-sync IO, Python/DELFIN footprint). '
                'When the cap is exceeded, the pool sequences submissions '
                'instead of letting two ORCA processes overshoot the SLURM '
                'allocation simultaneously.'
                '<br><br>'
                '<b>Method-dependent guidance</b><br>'
                '<table style="border-collapse:collapse; margin:4px 0;">'
                '<tr><th style="text-align:left; padding-right:14px;">Value</th>'
                '<th style="text-align:left; padding-right:14px;">Suitable for</th>'
                '<th style="text-align:left;">Notes</th></tr>'
                '<tr><td><code>0.95</code></td>'
                '<td>Pure DFT single-point and gradient (hybrid functionals, '
                'SVP–TZVP)</td>'
                '<td>Real RSS empirically ~80 % of <code>%maxcore × cores</code>. '
                'Headroom for OS/MPI overhead only.</td></tr>'
                '<tr><td><code>0.90</code></td>'
                '<td>All DFT including TDDFT, optimisation, frequency</td>'
                '<td><b>Default.</b> Balanced for parallel job pools.</td></tr>'
                '<tr><td><code>0.85</code></td>'
                '<td>Correlated methods (DLPNO-CCSD(T), MP2/RI-MP2 '
                'gradients, CASSCF), large active spaces</td>'
                '<td>Amplitude tensors and external-storage buffers can push '
                'real RSS over the <code>%maxcore</code> hint.</td></tr>'
                '<tr><td><code>≤ 0.80</code></td>'
                '<td>Heavy multi-reference, large excited-state manifolds, '
                'or repeated OOM-SIGKILLs (exit 137)</td>'
                '<td>Maximises sequencing safety at the cost of parallelism.</td></tr>'
                '</table>'
                '<br>'
                '<b>Failure mode this prevents:</b> Linux cgroup OOM-kill '
                'when the sum of concurrent ORCA processes\' resident-set '
                'sizes exceeds the SLURM memory grant. The cgroup-OOM is '
                'unconditional — a single byte over the limit terminates '
                'the entire job with SIGKILL and forfeits walltime.'
                '</div>'
            ),
            widgets.HBox(
