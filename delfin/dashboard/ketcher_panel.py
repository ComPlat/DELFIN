...
def read_js(scope: str, kind: str, want: str, *,
    ...
    *kind* travels back with the answer so the one channel can carry both
    ...
    return (
        ...
        "    catch(e){ arrow=false; }\n"
        "    /* An SDF for a scheme, because it is the only one of these that\n"
        "       keeps every component: measured on one canvas, getRxn gave 3\n"
        "       records, getCml 2 and getSdf 4, and the one getRxn dropped was\n"
        "       the reagent written over the first arrow.  The KET always,\n"
        "       because the arrows are only in there -- and because the\n"
        "       drawing itself is kept beside the job it was drawn for. */\n"
        "    return Promise.all([arrow ? api.getSdf() : api.getMolfile(),\n"
        "                        api.getKet()])\n"
