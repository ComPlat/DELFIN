...
CONTROL_FIELD_SPECS: Iterable[FieldSpec] = (
    ...
    FieldSpec("MANTA_TIME_BUDGET", _as_non_negative_float, default=1800.0,
             help="Wall-clock budget in seconds for building one complex. It is enforced: the construction runs in an isolated subprocess and that subprocess is killed, together with the UFF workers it spawned. A cut build returns nothing at all, not a smaller answer, so this is a real decision -- measured over 2000 systems, 1800 s cut 9.6 percent of 51-80 atom complexes and 39.6 percent of those above 80 atoms, which is the band holding porphyrins, terpyridines and cyclams. 3600 costs 2.5 and 15.6 percent; nothing in that sample exceeded 7200. Set 0 for no limit, which is right for a complete manifold on a heavy macrocycle: that is a long deterministic construction, not a hang."),
