...
class _WorkflowManager:
    ...
    def _plan_core_allocations(
        ...
        if ready_count == 1:
            # ORCA cannot grow to more cores after launch, so a single runnable job
            # should consume all currently available pool capacity immediately.
            # Holding cores back for jobs that are only runnable later leaves the
            # machine idle and permanently under-sizes the already running ORCA task.
            with self._lock:
                unfinished = (
                    set(self._jobs.keys())
                    - self._completed
