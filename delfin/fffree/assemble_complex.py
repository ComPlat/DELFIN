...
def _clash_count(Q, existing, syms_Q, syms_ex):
    ...
    Opt-in: when ``DELFIN_FFFREE_HH_CLASH_INCLUDE=1`` is set, ALSO
    counts inter-block H-H pairs below ``factor × (r_H + r_H)`` using
    the Bondi-style 0.85 fraction (so eclipsing methyl-methyl H-H at
    ~ 1.95-2.04 Å is flagged).  Default OFF -> byte-identical to HEAD
    ``93b396d`` (the 0.70 factor leaves H-H slack until 1.68 Å).
    """
    if len(existing) == 0:
        return 0
    ...
    for a in range(len(Q)):
        for b in range(len(existing)):
            ...
            if d < 0.70 * (_vdw(syms_Q[a]) + _vdw(syms_ex[b])):
                c += 1
    # Opt-in: tighter Bondi-vdW H-H floor at 0.85 (default OFF).
    try:
        from delfin.fffree.hh_clash_detector import (
            hh_clash_active as _hh_active,
            hh_clash_factor as _hh_factor,
            H_VDW_RADIUS as _r_H,
        )
        if _hh_active():
            fac_hh = float(_hh_factor())
            d_min_hh = fac_hh * (_r_H + _r_H)
            # Re-iterate H-H pairs (these are inter-block by construction
            # so geminal/1-3 within a ligand do not arise here).
            for a in range(len(Q)):
                if str(syms_Q[a]) != "H":
                    continue
                for b in range(len(existing)):
                    if str(syms_ex[b]) != "H":
                        continue
                    d = float(np.linalg.norm(Q[a] - existing[b]))
                    if d < d_min_hh and d >= 0.70 * (_vdw(syms_Q[a]) + _vdw(syms_ex[b])):
                        # Pair not already counted above; add it now.
                        c += 1
    except Exception:
        pass
    return c
