...
def grip_polish(
    ...
    *,
    ...
    # Opt-in: focused H-H clash penalty (User mandate 2026-06-04).
    # The generic ClashFloorPenalty already iterates H-H pairs whenever
    # the per-atom vdW table contains H (DEFAULT_VDW_RADII has r_H=1.20).
    # The extra term below is activated by
    # ``DELFIN_FFFREE_HH_CLASH_INCLUDE=1`` and adds a specialised
    # severity penalty using class-conditional topology filtering
    # (geminal + 1-3 H-H within CH3 are skipped, 1-4+ non-bonded are
    # flagged).  Default OFF -> byte-identical to HEAD ``93b396d``.
    _hh_clash_active_flag = False
    _hh_clash_weight_val = 0.0
    _hh_clash_factor_val = 0.85
    _hh_clash_min_topo_val = 4
    _hh_syms_cache: Optional[List[str]] = None
    try:
        from .hh_clash_detector import (
            hh_clash_active as _hh_active,
            hh_clash_factor as _hh_factor,
            hh_clash_min_topo as _hh_min_topo,
            hh_clash_weight as _hh_weight,
        )
        if _hh_active():
            _hh_clash_active_flag = True
            _hh_clash_weight_val = float(_hh_weight())
            _hh_clash_factor_val = float(_hh_factor())
            _hh_clash_min_topo_val = int(_hh_min_topo())
            _hh_syms_cache = [
                str(a.GetSymbol()) for a in mol.GetAtoms()
            ]
    except Exception:
        _hh_clash_active_flag = False
    ...
    def loss_and_grad(x_flat: np.ndarray) -> Tuple[float, np.ndarray]:
        ...
        G = G_grip + G_clash
        # Opt-in H-H penalty (env-flag DELFIN_FFFREE_HH_CLASH_INCLUDE).
        # Same one-sided quadratic shape as ClashFloorPenalty; uses a
        # topology-aware exclusion (geminal + 1-3 H-H skipped).
        if _hh_clash_active_flag and _hh_syms_cache is not None:
            try:
                from .hh_clash_detector import detect_hh_clashes
                hh_pairs = detect_hh_clashes(
                    _hh_syms_cache, R,
                    factor=_hh_clash_factor_val,
                    min_topo_distance=_hh_clash_min_topo_val,
                )
                if hh_pairs:
                    d_min_hh = _hh_clash_factor_val * (1.20 + 1.20)
                    L_hh = 0.0
                    w_hh = _hh_clash_weight_val
                    for (i, j, d, _sev) in hh_pairs:
                        if d < 1e-9:
                            continue
                        gap = d_min_hh - d
                        L_hh += w_hh * gap * gap
                ...
                # H-H penalty failure must never crash the polish; the
                # default heavy-pair gradient already covers the worst.
                pass
