...
# -------------------------------------------------------------------------------------------------------
def read_occupier_file(folder_name, file_name, multiplicity, additions, min_fspe_index, config): # multiplicity, additions, min_fspe_index could be removed in fkt call, will not be used, assigment and return are done by this fkt
    folder = Path(folder_name)
    if not folder.is_dir():
        print(f"Folder '{folder}' not found.")
        return None, None, None # or exit programm if None is not an option in Main as valide value
...
def prepare_occ_folder_2(folder_name, source_occ_folder, charge_delta=0, config=None):
    ...
    def _ensure_xyz_header(xyz_path: Path):
        with xyz_path.open("r", encoding="utf-8", errors="ignore") as xyz_file: # no use of shadow name f inside a nested fkt
