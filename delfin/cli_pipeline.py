    ...
    Supports flow control types in YAML::
        ...
        steps:
          ...
          - step: orca_opt
            ...
            charge: 0
    """
    # Single source of truth for "dict -> pipeline": the framework serializer.
    # The YAML format is a subset of the PipelineSpec it accepts, so existing
    # pipeline files keep loading unchanged.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        # Static validation (informational — never changes the exit code, so an
        # unresolved template or an unknown step is surfaced without aborting).
        try:
            from delfin.tools.pipeline import PipelineTemplate
            geom = bool(getattr(args, "geometry", None))
            if isinstance(pipe, PipelineTemplate):
                report = pipe.validate(geometry=geom, **_parse_params(args.param))
            else:
                report = pipe.validate(geometry=geom)
            print()
            print(report.summary())
        except Exception as exc:  # never let validation break the dry run
